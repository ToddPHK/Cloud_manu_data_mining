...
namespace DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static string connectionString3 = PubConstant.ConnectionString3;
        ...
        public static string switchConnectString(int whichDB)
        {
            string tmp_connectionString;
            switch (whichDB)
            {
                case 2:
                    tmp_connectionString = connectionString2;
                    break;
                case 3:
                    tmp_connectionString = connectionString3;
                    break;
                default:
                    tmp_connectionString = connectionString;
                    break;
            }
        ...
        public static int ExecuteSql(int whichDB,string SQLString)
        {
            string tmp_connectionString=switchConnectString(whichDB);
        ...
        /// 执行一次SQL语句
        /// </summary>
        /// <param name="whichDB">选择执行SQL的数据库对象.</param>
        ...
        {
            string tmp_connectionString;
            tmp_connectionString = "Data Source=127.0.0.1,7711;User ID=sa;Password=bd031518;DataBase=" + dbName + ";";
            using (SqlConnection connection = new SqlConnection(tmp_connectionString))
            {
                using (SqlCommand cmd = new SqlCommand(SQLString, connection))
                {
        ...
        public static SqlDataReader ExecuteReader(int whichDB,string strSQL)
        {
            string tmp_connectionString = switchConnectString(whichDB);
            SqlConnection connection = new SqlConnection(tmp_connectionString);
            SqlCommand cmd = new SqlCommand(strSQL, connection);
            try
            {
                connection.Open();
        ...
        /// <summary>
        /// Queries the specified which DB.
        /// </summary>
        /// <param name="whichDB">选择执行查询的数据库</param>
        /// <param name="SQLString">The SQL string.</param>
        /// <returns>DataSet.</returns>
        public static DataSet Query(int whichDB,string SQLString)
        {
            string tmp_connectionString=switchConnectString(whichDB);
        ...
        public static DataTable FindByConditions(int whichDB, object obj, string conditions)
        {
            string sql = "select * from " + Common<object>.GetModelInfo(obj)[0] + (conditions.Trim().Length <= 0 ? "" : " where " + conditions);
            return Query(whichDB,sql).Tables[0];
        }

        /// <summary>
        /// 按sql语句查询并返回Table
        /// </summary>
        /// <param name="whichDB">The which DB.</param>
        /// <param name="sql">The SQL.</param>
        /// <returns>DataTable.</returns>
        public static DataTable FindTable(int whichDB, string sql)
        {
            return Query(whichDB, sql).Tables[0];
        }

        public static DataTable FindTable(string dbName, string sql)
        {
            return Query(dbName, sql).Tables[0];
        }
        /// <summary>
        ...
        public static DataTable FindByPage(int whichDB, object obj, string start, string limit, string conditions)
        {
            string[] ModelInfo = Common<object>.GetModelInfo(obj);
            string sql = "select top " + limit + " * from " + ModelInfo[0] + " where " + ModelInfo[1] + " <( select isnull(MIN(" + ModelInfo[1] + "),(select max(" + ModelInfo[1] + ")+1 from " + ModelInfo[0] + ")) from (select top " + start + " " + ModelInfo[1] + " from " + ModelInfo[0] + " order by " + ModelInfo[1] + " desc) as Temp ) order by " + ModelInfo[1] + " desc";
            if (conditions.Trim().Length > 0)
        ...
        public static DataTable FindFieldsByPage(int whichDB, object obj, string fields, string start, string limit, string conditions)
        {
            string[] ModelInfo = Common<object>.GetModelInfo(obj);
            string sql = "select top " + limit +" " + fields + " from " + ModelInfo[0] + " where " + ModelInfo[1] + " <( select isnull(MIN(" + ModelInfo[1] + "),(select max(" + ModelInfo[1] + ")+1 from " + ModelInfo[0] + ")) from (select top " + start + " " + ModelInfo[1] + " from " + ModelInfo[0] + " order by " + ModelInfo[1] + " desc) as Temp ) order by " + ModelInfo[1] + " desc";
            if (conditions.Trim().Length > 0)
            {
                sql = "select top " + limit + " " + fields + " from " + ModelInfo[0] + " where " + ModelInfo[1] + " <( select isnull(MIN(" + ModelInfo[1] + "),(select max(" + ModelInfo[1] + ")+1 from " + ModelInfo[0] + ")) from (select top " + start + " " + ModelInfo[1] + " from " + ModelInfo[0] + "  where " + conditions + " order by " + ModelInfo[1] + " desc) as Temp ) and (" + conditions + ") order by " + ModelInfo[1] + " desc";
            }
            return Query(whichDB, sql).Tables[0];
        }
        /// <summary>
        ...
        public static string GetCountByConditions(int whichDB, object obj, string conditions)
        {
            string sql = "select count(*) from " + Common<object>.GetModelInfo(obj)[0];
            if (conditions.Trim().Length > 0)
                sql += " where " + conditions;
            try
            {
                return Query(whichDB, sql).Tables[0].Rows[0][0].ToString();
            }
            catch (Exception error)
            {
                log.Error("GetCountByConditions:" + error.Message);
        ...
        public static bool Insert(object obj)
        {
            ...
            for (int i = 0; i < pis.Length; i++)
            {
                if (pis[i].Name.ToString().ToLower() != ModelInfo[1].ToLower())
                {
                    if (pis[i].GetValue(obj, null).ToString() != "0001/1/1 0:00:00" && pis[i].GetValue(obj, null).ToString() != "0001-1-1 0:00:00")//时间未填
                    {
