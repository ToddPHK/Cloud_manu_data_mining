...
var makeTab = function (id, url, title) {
    ...
    tab = App.CenterPanel.add(new Ext.Panel({
        id   : id,        
        tbar : [{
            text      : "Source Code",
            iconCls   : "#PageWhiteCode",
    ...
    tab.sWin = win;
    setTimeout(function(){
        App.CenterPanel.setActiveTab(tab);
    }, 250);
...
var loadExample = function (href, id, title) {
    var tab = App.CenterPanel.getComponent(id),
        lObj = lookup[href];
...
var change = function (token) {
    if (!lockHistoryChange) {
        if (token) {
            ...
            App.CenterPanel.setActiveTab(0);
        }
    }
    lockHistoryChange = false;
};
...
var changeFilterHash = Ext.Function.createBuffered(
    function (text) {
        ...
        if (text.length > 2) {
            ...
            var tab = App.CenterPanel.getActiveTab(),
                token = "";
...
var themeChange = function (menu, menuItem) {
    var href = window.location.href;
        index = href.indexOf("#"),
        reload = menuItem.text == "Neptune" || Ext.net.ResourceMgr.theme == "neptune";
    ...
    App.direct.GetThemeUrl(menuItem.text, reload, {
		success : function (result) {
            ...
            App.CenterPanel.items.each(function (el) {
...
var updateComments = function (updateCount, url) {
    ...
    Ext.net.DirectMethod.request({
        ...
        success      : function (result, response, extraParams, o) {
            ...
            if (updateCount) {
                App.CenterPanel.getActiveTab().commentsBtn.setText("Comments (" + result.length + ")");
            }
        },
